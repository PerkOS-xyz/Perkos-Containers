...
#   PERKOS_RUNTIME              "hermes" | "openclaw" | "zeroclaw" (manifest + excludes)
...
case "$RUNTIME" in
  hermes)
    ...
    ;;
  zeroclaw)
    # config.toml is re-rendered from env on every boot AND carries ZeroClaw's
    # own encrypted secrets (`enc2:…`, including the LLM key and the pre-seeded
    # gateway token). Restoring a stale copy would both revert a re-provisioned
    # key and risk carrying ciphertext whose key material may not travel to a
    # different task — the exact failure the Hermes/OpenClaw excludes exist to
    # prevent. Same reasoning for the minted gateway bearer.
    EXCLUDES+=( --exclude='./.zeroclaw/config.toml' --exclude='./.zeroclaw/.gateway-token' )
    ;;
  *)
